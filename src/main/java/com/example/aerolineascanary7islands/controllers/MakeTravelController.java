...
public class MakeTravelController implements Initializable {
     ...
     * Comprueba también que la fecha salida sea anterior a la fecha llegada llamando al método validarFechas de la clase Vuelo y
     * que las fechas sean validas llamando a validarValoresFecha.
                   ...
                   if(cB_procedencia.getValue()!=cB_destino.getValue()) {
                       if(tF_fecha_Llegada.getText().matches(expRegularFecha) && tF_fecha_Salida.getText().matches(expRegularFecha)) {
                           if(Vuelo.validarFechas(tF_fecha_Salida.getText(),tF_fecha_Llegada.getText())){
                               if(Vuelo.validarValoresFecha(tF_fecha_Salida.getText()) && Vuelo.validarValoresFecha(tF_fecha_Llegada.getText())) {
                                   ...
                                   MethodsForControllers.showAlert("Error","Fechas invalidas", Alert.AlertType.ERROR);
                               }
                           }else{
